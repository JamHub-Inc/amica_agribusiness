...
import { Search, Bell, Plus, ChevronDown, Menu, UserIcon, LogOut, Eye, Settings, Shield, Users, BarChart3, ScrollText } from 'lucide-react';
...
const AdminHeader = ({ toggleSidebar }) => {
  ...
  const unreadCount = 5;
  const notifications = [
    { id: 1, title: 'New User Registration', message: 'A new farmer has registered and awaits approval.', status: 'UNREAD', createdAt: new Date() },
    { id: 2, title: 'Loan Approval Needed', message: 'KES 150,000 loan request from Kinangop SACCO.', status: 'UNREAD', createdAt: new Date() },
    { id: 3, title: 'Price Update Alert', message: 'Maize prices have exceeded threshold limits.', status: 'READ', createdAt: new Date() },
  ];
  ...
  const quickActions = [
    { label: 'Manage Users', icon: Users, href: '/admin/users', color: 'from-indigo-500 to-violet-500', description: 'Add, edit, or manage users' },
    { label: 'View Reports', icon: BarChart3, href: '/admin/reports', color: 'from-emerald-500 to-teal-500', description: 'System-wide analytics' },
    { label: 'Activity Logs', icon: ScrollText, href: '/admin/activity-logs', color: 'from-amber-500 to-orange-500', description: 'Audit trail and security' },
  ];

  const handleSearch = (e) => setSearchQuery(e.target.value);
  const handleKeyDown = (e) => { if (e.key === 'Enter' && searchQuery.trim()) console.log('Admin search:', searchQuery); };
  ...
  return (
    <header className="sticky px-4 py-4 top-0 z-40 flex h-20 items-center justify-between gap-4 border-b border-border/40 bg-gradient-to-r from-background/95 to-background/90 backdrop-blur-xl supports-[backdrop-filter]:bg-background/80 shadow-sm transition-all duration-300">
      <div className="flex items-center gap-4">
        ...
        {/* Admin Badge */}
        <div className="hidden md:flex items-center gap-2 bg-primary/5 px-3 py-1.5 rounded-xl border border-primary/10">
          <Shield className="h-3.5 w-3.5 text-primary" />
          <span className="text-[10px] uppercase tracking-[0.15em] font-medium text-primary">System Admin</span>
        </div>

        <div className="hidden lg:flex flex-1 max-w-md">
          <div className="relative w-[320px]">
            <Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/70" />
            <Input placeholder="Search users, loans, produce..." className="pl-10 pr-4 h-10 rounded-xl bg-muted/50 border-border/50 focus-visible:ring-primary/40 text-sm shadow-none transition-all hover:bg-muted/80 focus:bg-background" value={searchQuery} onChange={handleSearch} onKeyDown={handleKeyDown} />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 lg:gap-3">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64 p-2 rounded-2xl border-border/50 shadow-xl backdrop-blur-md bg-background/95">
            <DropdownMenuLabel className="text-xs font-medium text-muted-foreground uppercase tracking-widest px-2 py-2">Admin Shortcuts</DropdownMenuLabel>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-80 p-2 rounded-2xl border-border/50 shadow-xl backdrop-blur-md bg-background/95">
            <div className="flex items-center justify-between px-3 py-3">
              <DropdownMenuLabel className="text-xs font-bold text-muted-foreground uppercase tracking-widest p-0">System Notifications</DropdownMenuLabel>
            ...
            <DropdownMenuItem className="justify-center text-sm font-bold text-primary hover:text-primary transition-colors cursor-pointer py-3 rounded-xl bg-primary/5 hover:bg-primary/10 mt-1">
              <Eye className="h-4 w-4 mr-2" />View all notifications
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative rounded-2xl h-12 px-2 gap-3 hover:bg-secondary/10 border border-transparent hover:border-secondary/20 transition-all">
              <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center text-primary-foreground font-medium text-sm shadow-sm">
                {userInitials}
              </div>
              <div className="hidden md:flex flex-col items-start">
                <span className="text-sm font-medium text-foreground leading-none mb-1">{user?.name || 'Admin'}</span>
                <span className="text-[10px] font-medium uppercase tracking-widest text-muted-foreground leading-none">System Admin</span>
              </div>
          ...
          <DropdownMenuContent align="end" className="w-60 p-2 rounded-2xl border-border/50 shadow-xl backdrop-blur-md bg-background/95 mt-2">
            <div className="px-3 py-3">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center text-primary-foreground font-black text-lg shadow-sm">
                  {userInitials}
                </div>
                <div className="flex flex-col"><span className="text-sm font-bold text-foreground">{user?.name || 'Admin'}</span><span className="text-xs font-medium text-muted-foreground/80 truncate w-[140px]">{user?.email || 'admin@Amica.co.ke'}</span></div>
              </div>
            </div>
            <DropdownMenuSeparator className="my-1 bg-border/40" />
            <DropdownMenuItem className="p-3 rounded-xl hover:bg-muted/60 focus:bg-muted/60 cursor-pointer gap-3 transition-colors" asChild>
              <Link to="/admin/profile"><UserIcon className="h-4 w-4 text-muted-foreground" /><span className="text-sm font-semibold">My Profile</span></Link>
            </DropdownMenuItem>
            <DropdownMenuItem className="p-3 rounded-xl hover:bg-muted/60 focus:bg-muted/60 cursor-pointer gap-3 transition-colors" asChild>
              <Link to="/admin/settings"><Settings className="h-4 w-4 text-muted-foreground" /><span className="text-sm font-semibold">System Settings</span></Link>
            </DropdownMenuItem>
            <DropdownMenuSeparator className="my-1 bg-border/40" />
